...
void SumMatrixRowinArray(int arr[3][3], int arrsum[3], short row, short colems)
{
	for (int i = 0; i < colems; i++)
	{
		arrsum[i] = RowSum(arr, i, colems);
	}
}

void PrintRowSum(int arrsum[3],short row)
{
	for (int i = 0; i < row; i++)
	{
		cout << "Row " << i + 1 << " sum is " <<arrsum[i];
		cout << endl;
