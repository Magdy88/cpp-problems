...
string ReadDateinString()
{
	string Date;
	cout << "Please Inter DAte dd//mm//yyyy?";
	getline(cin>>ws,Date);
...
stDate ConvertLinetoRecord(string line, string seperator = "/")
{
	stDate Date;
	vector<string>vString;
...
string ConvertRecordToLine(stDate Date, string Seperator =
	"/")
{
	return to_string(Date.Day) + "/" + to_string(Date.Month) + "/" +to_string (Date.Year);

}
string ReplaceWordInString(string S1,string stringtoReplace,string ReplacedTo)
{
	short pos = S1.find(stringtoReplace);

	while (pos != S1.npos)
	{
		S1 = S1.replace(pos, stringtoReplace.length(), ReplacedTo);
...
string FormateDate(stDate Date, string DateFormate = "dd,mm,yyyy")
{
	...
	FormateDateString = ReplaceWordInString(DateFormate, "dd", to_string(Date.Day));
...
int main()
{
	 ...
	 stDate Date = ConvertLinetoRecord(DateString);
	 cout << ConvertRecordToLine(Date) << endl;
