...
stDate GetActucalVavatoionDays(stDate Date1, short VacationDay)
{
	...
	for (short i = 1; i <= VacationDay + WeekEndCounter; i++)
	{
		if (IsWeekEnd(Date1))
			WeekEndCounter++;
...
int main()
{
	...
	short VacationDay = ReadDay();


	cout << "Vacatoion From : " << DayShortName(DayOfWeekOrder(Date1)) << " , "
		<< Date1.Day << "/" << Date1.Month << "/" << Date1.Year << "\n";

	stDate ReturnDate = GetActucalVavatoionDays(Date1, VacationDay);

	cout << "Vacatoion From : " << DayShortName(DayOfWeekOrder(ReturnDate)) << " , "
